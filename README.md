...
- Le dossier `files/` est monté en lecture seule pour servir des fichiers statiques via `--files-dir /app/files` (les fichiers sont accessibles à `http://<host>:<port>/gradio_api/file=/app/files/<nom-du-fichier>`)
...
# Servir un répertoire de fichiers statiques via des URLs Gradio
# Les fichiers seront accessibles à http://<host>:<port>/gradio_api/file=<chemin-absolu>/<fichier>
# Voir https://www.gradio.app/guides/file-access pour plus de détails
prevmed --survey-yaml <yaml> --scoring-script <script> --files-dir /chemin/vers/fichiers
```
...
# Chaque entrée accepte les mêmes clés que la config principale (sauf
# "questions") plus une clé "route" obligatoire.
# Les valeurs "body" et "header" peuvent être du Markdown inline OU un
# chemin vers un fichier .md (absolu, ou relatif à ce fichier YAML).
extra_pages:
  - route: accueil
...
questions:
  - variable: nom_variable
    ...
    widget_args:
      ...
      step: 1  # Pour Slider
      value: 0  # Valeur par défaut du widget (optionnel)
...
│       ├── ProbaLYNCH.R         # Script de scoring ProbaLYNCH (R)
│       ├── ProbaLYNCH.py        # Script de scoring ProbaLYNCH (Python)
│       ├── style.css            # CSS personnalisé
│       ├── extra.js             # JavaScript supplémentaire
│       └── md/                  # Fichiers Markdown (body, etc.)
