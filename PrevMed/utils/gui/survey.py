...
a greetings landing page (from ``gui/greetings.py``) is shown first;
the survey content is hidden until the user clicks "Start".
"""
...
def create_survey_interface(
    ...
    with gr.Blocks(
        ...
        # When greetings_md is in the YAML, show a landing page first;
        # the survey is hidden until the user clicks "Start".
        has_greetings = bool(config.get("greetings_md"))
        greetings_col = None
        start_btn = None

        if has_greetings:
            greetings_col, start_btn = create_greetings_section(
        ...
        # --- Survey content ---
        # Wrapped in a Column so it can be hidden behind the greetings page.
        # When there is no greetings page, the survey is visible immediately.
        with gr.Column(visible=not has_greetings) as survey_col:
        ...
        # --- Wire start button to toggle greetings -> survey ---
        if has_greetings and start_btn is not None:
            def show_survey():
                """Hide greetings, show survey."""
                return gr.update(visible=False), gr.update(visible=True)

            start_btn.click(
                fn=show_survey,
                outputs=[greetings_col, survey_col],
                show_progress="hidden",
            )
